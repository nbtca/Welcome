...
export type CaseCreator = (
  count?: number
) => Generator<ProgressStat, any, boolean>;
...
function getRateOfStats(stats: ProgressStat[]): number {
  return (
    stats
      .map(({ duration, size }) => (size / (duration + 0.00001)) * 1000)
      .reduce((acc, rate) => acc + rate, 0) / stats.length
  );
}

export function createFiber(
  request: (count?: number) => Observable<number>
): FiberCreator {
      ...
      const timerSub = timer.subscribe({
        next() {
          sub.next(
            fiberInfos
              .map((info) => info.calc())
              .reduce((acc, rate) => acc + rate, 0) * fixFactor
          );
          if (count++ >= maxCount) {
            unsubscribe();
          }
